defmodule AuthN.Ecto.Accounts do
  defmacro __using__(opts) do
    ecto_repo = Keyword.fetch!(opts, :ecto_repo) |> Macro.expand(__ENV__)
    user_schema = Keyword.fetch!(opts, :user_schema) |> Macro.expand(__ENV__)
    user_notifier = Keyword.fetch!(opts, :user_notifier) |> Macro.expand(__ENV__)
    email_field = Keyword.get(opts, :email_field, :email)

    quote do
      import Ecto.Query, warn: false

      alias unquote(ecto_repo), as: Repo
      alias unquote(user_schema), as: User
      alias unquote(user_notifier), as: UserNotifier
      alias unquote(Module.concat([user_schema, Token])), as: UserToken
      ...
      def get_user_by_email(email) when is_binary(email) do
        Repo.get_by(User, [{unquote(email_field), email}])
      end
      ...
      def register_user(attrs) do
        %User{}
        |> User.registration_changeset(attrs, unquote(ecto_repo))
        |> Repo.insert()
      end
      ...
      def change_user_registration(%User{} = user, attrs \\ %{}) do
        User.registration_changeset(user, attrs, unquote(ecto_repo))
      end
      ...
      def change_user_email(user, attrs \\ %{}) do
        User.email_changeset(user, attrs, unquote(ecto_repo))
      end
      ...
      def apply_user_email(user, password, attrs) do
        user
        |> User.email_changeset(attrs, unquote(ecto_repo))
      ...
      defp user_email_multi(user, email, context) do
        changeset = user |> User.email_changeset(%{unquote(email_field) => email}, unquote(ecto_repo)) |> User.confirm_changeset()
